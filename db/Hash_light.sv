...
module hash_function(
    ...
    // Istanziare i moduli
    SA sa_inst (.m(m), .IV(IV), .H(H_intermediate));
    XOR_Module xor_inst (.H_in(H), .IV(IV), .H_out(H_intermediate));
    Theta theta_inst (.H_in(H_intermediate), .H_out(H_intermediate));
    rho rho_inst (.H_in(H_intermediate), .H_out(H));
